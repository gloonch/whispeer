...
function NavIcon({ name, active }) {
  ...
  if (name === "explore") {
    return (
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={active ? "2.25" : "1.5"} stroke="currentColor" className={className} aria-hidden="true">
...
export default function NavBar() {
  ...
  const items = [
    { to: "/whispers", label: "Whispers", icon: "whispers" },
        ...
        {items.map((item, index) => {
          ...
          return (
            <NavLink
              ...
              className={`relative flex h-16 items-center justify-center transition hover:bg-slate-950/5 ${active ? "text-slate-950" : "text-slate-600"}`}
            >
              {index > 0 ? <span className="absolute left-0 top-1/2 h-8 -translate-y-1/2 border-l border-slate-300/80" /> : null}
              <span className="grid h-10 w-10 place-items-center transition">
